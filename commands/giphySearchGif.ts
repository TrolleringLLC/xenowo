...
module.exports = {
  data: new builder.xenoBuilder("giphyGif", "search giphy for a gif!", "image"),
  execute: (msg: Message, bot: Client) => {
    const args: string = msg.content.replace(
      xenowuLib.retrieveSetting("prefix") + "giphyGif ",
      ""
    );
    if (
    ...
    axios
      .get(
        `https://api.giphy.com/v1/gifs/search?api_key=${xenowuLib.retrieveSetting(
          "giphyApiKey"
        )}&q=${args}&limit=1`
      )
      .then((resp: any) => {
        ...
        msg.channel.send(resp.data.data[0].url);
      });
  },
};
